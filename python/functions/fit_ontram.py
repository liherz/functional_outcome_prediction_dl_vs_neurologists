...
def fit_ontram(model, y_train, x_train = None, x_train_im = None, x_test = None, x_test_im = None, y_test = None,
               ...
               augment_batch = None, multilabel = False, combined_loss = False):
    ...
    for epoch in range(epochs):
        ...
        for i in range(int(n/batch_size)):
            ...
            if(model.nn_x != None):
                #p = x_train@np.linalg.inv(x_train.T@x_train)@x_train.T
                #i = np.identity(x.shape[0])
                #q = i-p
                #p_batch = p[batch_idx[batch_tmp:(batch_tmp + batch_size)],:][:,batch_idx[batch_tmp:(batch_tmp + batch_size)]]
                #x_batch = [x_batch, p_batch]
            ...
            if(combined_loss):
                ...
                loss_value, grads = apply_gradient(model, bl = x_batch_bl, x = x_batch, 
                                                   x_im = x_batch_im, y = y_batch, 
        ...
        if(y_test is not None):
            ...
            for i in range(int(n_test/batch_size)):
                ...
                if(model.nn_x != None):
                    #p_test = x_test@np.linalg.inv(x_test.T@x_test)@x_test.T
                    #i = np.identity(x.shape[0])
                    #q = i-p
                    #p_batch_test = p_test[batch_idx_test[batch_tmp:(batch_tmp + batch_size)],:][:,batch_idx_test[batch_tmp:(batch_tmp + batch_size)]]
                    x_batch_test = x_test[batch_idx_test[batch_tmp:(batch_tmp + batch_size)]]
                    #x_batch_test = [x_batch_test, p_batch_test]
                if(model.nn_im != None):
            ...
            if(output_dir is not None): # save all models
